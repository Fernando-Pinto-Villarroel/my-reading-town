...
class FirstOrLastLineScreen extends StatefulWidget {
  ...
  @override
  State<FirstOrLastLineScreen> createState() => _FirstOrLastLineScreenState();
}

class _FirstOrLastLineScreenState extends State<FirstOrLastLineScreen> {
  List<Map<String, dynamic>> _questions = [];
  final Random _random = Random();
  int _consecutiveWins = 0;
  static const String _minigameId = 'first_or_last_line';
  ...
  Future<void> _loadQuestions() async {
    ...
    final jsonStr = await rootBundle
        .loadString('assets/messages/$locale/first_or_last_line.json');
  ...
  void _selectAnswer(bool answer) {
    ...
    final correctIsFirst = _currentQuestion!['is_first'] as bool;
    final isCorrect = answer == correctIsFirst;
  ...
  Future<void> _onGameWon() async {
    ...
    if (mounted) {
      ...
      final name = lang.translate('first_or_last_line');
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              return Column(
                children: [
                  MinigameTopBar(
                    title: context.t('first_or_last_line'),
      ...
      child: Column(
        children: [
          ...
          QuestionBubble(
            ...
            subtitle: context.t('is_this_first_or_last'),
            questionText: _currentQuestion!['line'] as String,
          ),
          const SizedBox(height: 8),
          _BookLabel(book: _currentQuestion!['book'] as String),
          const SizedBox(height: 24),
          _BinaryChoiceRow(
            leftLabel: context.t('first_line'),
            rightLabel: context.t('last_line'),
            leftIcon: Icons.format_quote,
            rightIcon: Icons.last_page,
            onLeftTap: _selectedAnswer == null ? () => _selectAnswer(true) : null,
            onRightTap: _selectedAnswer == null ? () => _selectAnswer(false) : null,
          ...
          if (_showResult) ...[
            ...
            _BinaryResultFeedback(
              ...
              consecutiveWins: _consecutiveWins,
              winsNeeded: _config.winsNeeded,
              correctLabel: (_currentQuestion!['is_first'] as bool)
                  ? context.t('first_line')
                  : context.t('last_line'),
            ),
          ],
          const SizedBox(height: 16),
  ...
  _BinaryButtonState _buttonState(bool isFirst) {
    if (_selectedAnswer == null) return _BinaryButtonState.idle;
    final correctIsFirst = _currentQuestion!['is_first'] as bool;
    if (isFirst == correctIsFirst) return _BinaryButtonState.correct;
    if (_selectedAnswer == isFirst) return _BinaryButtonState.wrong;
...
class _BookLabel extends StatelessWidget {
  final String book;
  const _BookLabel({required this.book});
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          Icon(Icons.menu_book, size: 16, color: _darkAccent),
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  fontSize: 14,
                  fontWeight: FontWeight.bold,
                  color: isCorrect ? const Color(0xFF2E7D32) : Colors.red.shade700,
                ),
              ),
            ],
          ),
