...
class _BookSearchDialogState extends State<BookSearchDialog> {
  ...
  List<GoogleBookResult> _results = [];
  bool _loading = false;
  String? _error;
  Timer? _debounce;
  int _searchId = 0;
  ...
  Future<void> _search(String query) async {
    ...
    try {
      final results = await GoogleBooksService.searchBooks(trimmed);
  ...
  void _selectResult(GoogleBookResult result) {
    // Open the form immediately, download cover in background
  ...
  Widget _resultTile(GoogleBookResult result) {
...
/// Wrapper that opens BookFormDialog immediately and downloads cover in background
class _PrefillBookForm extends StatefulWidget {
  final GoogleBookResult result;
...
class _PrefillBookFormState extends State<_PrefillBookForm> {
  ...
  Future<void> _downloadCover() async {
    ...
    final path = await ImageService.saveImageFromUrl(widget.result.thumbnailUrl!);
    if (!mounted) return;
    setState(() {
      _coverPath = path;
      _downloading = false;
