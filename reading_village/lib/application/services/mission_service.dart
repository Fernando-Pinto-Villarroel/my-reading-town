...
class MissionService {
  ...
  Future<Map<String, MissionProgress>> loadMissionProgress() async {
    ...
    return {
      for (final m in maps) m['mission_id'] as String: MissionProgress.fromMap(m),
    };
  }

  bool isBranchUnlocked(MissionBranch branch, Map<String, MissionProgress> progress) {
  ...
  bool isBranchFullyCompleted(MissionBranch branch, Map<String, MissionProgress> progress) {
  ...
  Mission? getActiveMission(MissionBranch branch, Map<String, MissionProgress> progress) {
  ...
  MissionProgress _getOrCreateProgress(Mission mission, Map<String, MissionProgress> progress) {
  ...
  Future<void> _ensureMissionActivated(Mission mission, Map<String, MissionProgress> progress) async {
    final p = _getOrCreateProgress(mission, progress);
    if (p.activatedAt == null) {
      p.activatedAt = DateTime.now().toIso8601String();
      await _invRepo.upsertMissionProgress(mission.id, activatedAt: p.activatedAt);
    }
  }
  ...
  bool _checkMissionCondition(Mission mission, List<PlacedBuilding> buildings,
      List<Villager> villagers, List<ActivePowerup> activePowerups,
      bool bookItemUsedSinceActive, {int? totalPagesRead, int? completedBooks}) {
    switch (mission.conditionType) {
      case MissionConditionType.buyBuilding:
        return buildings.any((b) => b.type == mission.buildingType && b.isConstructed);
      ...
      case MissionConditionType.villagerHappinessNatural:
        final hasAnyBookPowerup = activePowerups.any((p) => p.type == 'book_happiness' && p.isActive);
  ...
  ({int current, int target}) getMissionProgressValues(Mission mission,
      List<PlacedBuilding> buildings, List<Villager> villagers,
      List<ActivePowerup> activePowerups, bool bookItemUsedSinceActive,
    ...
    switch (mission.conditionType) {
      case MissionConditionType.buyBuilding:
        current = buildings.where((b) => b.type == mission.buildingType && b.isConstructed).length;
      ...
      case MissionConditionType.upgradeBuilding:
        ...
        current = maxLevel;
        return (current: current.clamp(0, mission.targetLevel ?? 1), target: mission.targetLevel ?? 1);
  ...
  Future<bool> claimMissionReward(String missionId, Map<String, MissionProgress> progress) async {
